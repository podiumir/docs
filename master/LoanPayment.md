...
لازم به ذکر است تغییر طرج بازپرداخت، تنها برای یک‌بار امکان‌پذیر می‌باشد.
...
|guarantorCode|می‌تواند یکی از کدهای زیر باشد: **LOAN_GUARANTOR_NO_GUARANTOR**  **LOAN_GUARANTOR_PASARGAD**  اولی یعنی بدون ضامن و دومی با ضمانت بانک پاسارگاد است|
...
|installmentStatusCode|در صورت وجود حتی یک قسط با این وضعیت، اطلاعات کامل تسهیلات و تمام اقساط آن برگردانده می‌شود.وضعیت اقساط می‌تواند یکی از کدهای زیر باشد:INSTALLMENT_STATUS_CANCELED  (لغو شده)   INSTALLMENT_STATUS_DONE  (پرداخت شده)   INSTALLMENT_STATUS_WAITING_FOR_BANK_SETTLEMENT  (منتظر تسویه بانکی)    INSTALLMENT_STATUS_PRE_SETTLED  (تسویه زودتر از موعد)   INSTALLMENT_STATUS_CREATED  (ایجاد شده)    INSTALLMENT_STATUS_DELETED  (حذف شده) 
...
|این سفارش به علت عدم تایید فروشگاه در بازه زمانی مشخص، برگشت خورده است و اقساط آن لغو شده اند.|  
کنسل شده|INSTALLMENT_STATUS_CANCELED|
...
در صورت وارد نکردن مقدار پارامترهای breathingPeriodType و breathingPeriodCount، تاریخ سرررسید اقساط از همین امروز محاسبه خواهد گردید.
