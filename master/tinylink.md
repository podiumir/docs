...
- [ملاقات لینک /متن /فایل](#menu)
- [لیست لینک ها/ متن ها/ فایل ها](#menu)
- [فعال / غیرفعال سازی لینک](#menu)
...
* shortenObjectKind (اجباری): نوع لینک کوتاه شده. دارای file,link یا content
...
این سرویس لیستی از تمام لینک‌های/متن‌های/فایل‌های کاربر را برمیگرداند. این سرویس قابلیت pagination نیز دارد، به این صورت که offset و count را نیز از ورودی دریافت می‌کند.
...
## فعال / غیرفعال سازی لینک

از این سرویس برای غیرفعال/فعال کردن لینک استفاده میشود.
* شناسه سرویس:  46413
* پارامترهای ورودی:
	* hash: کد یکتا که نماینده یک لینک/متن/فایل می‌باشد.
	* setStatus: فعال (مقدار active) یا غیرفعال (deactive) کردن لینک
* پاسخ برگشتی:
...
* شناسه سرویس: 46414
* پارامترهای ورودی:
	* hash (اجباری): کد یکتا که نماینده یک لینک/متن می باشد.
	* kind: یک رشته که نشان دهنده نوع آمار درخواستی می باشد. minute برای آمار دقیقه ای، hour برای آمار ساعتی، day برای آمار روزانه و month برای آمار ماهانه می‌باشد. بصورت پیش فرض مقدار month در نظر گرفته می‌شود.
	* fromTimestamp: زمان ابتدا جهت گزارش‌گیری
	* toTimestamp: زمان انتها جهت گزارش‌گیری
